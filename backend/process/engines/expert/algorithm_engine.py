...
包含 20 个参数的物理推导逻辑。
由 ProcessInitializer 编排层调用，不直接暴露给外部。

保留原有章节注释和 docstring，确保设计意图显式表达。
"""
...
class AlgorithmEngine:
    ...
    def __init__(
        ...
        """
        简化的初始化（由 ProcessInitializer 注入属性）
    ...
    def derive(self, params: ProductionParams) -> None:
        """
        执行所有算法推导

        Args:
            params: ProductionParams 实例，会被就地修改
        """
        # 按设备分组推导
    ...
    def _get_shrink_rate(self, c_inj: Dict[str, Any]) -> tuple:
        ...
        # ====== 第 2 级：按大类查表 ======
        family = parse_material_family(self.material.get('abbreviation', ''))
    ...
    def _get_family_meter_viscosity(self, abbreviation: str, c_met: Dict[str, Any]) -> Tuple[float, str, str]:
        ...
        family = parse_material_family(abbrev)
        if family and family in family_map:
    ...
    def _get_family_fill_efficiency(self, family: str, c_inj: Dict[str, Any]) -> Tuple[float, str]:
        """
        获取 family 填充效率（2 级兑底，不使用 Polymer 字段覆盖）

        物理含义：family 在实际注射中能达到的速度比例（实际速度 / 机器最大速度）
        - PP/PE 高流动性 → 0.85（可接近 max）
        - PC 高黏度 → 0.40（必须慢）
        - PVC 极易分解 → 0.30（最慢）

        优先级：
        1. 第 1 级 - family_fill_efficiency[family]（按材料查表）
        2. 第 2 级 - default_fill_efficiency（兑底）

        设计决策：不使用 Polymer 字段覆盖
        - 填充效率反映“材料物理极限”，不随制品调整
        - Polymer 的 recommend_inj_time_window 已覆盖制品级修正
        - 避免数据冲突，保持设计一致
        ...
        Returns:
            (fill_efficiency, level) 元组
            level ∈ {'family', 'default'}
        """
        # ====== 第 1 级：按 family 查表 ======
        family_efficiency = c_inj.get('family_fill_efficiency', {})
        if family and family in family_efficiency:
            return float(family_efficiency[family]), 'family'

        # ====== 第 2 级：默认值（兑底）======
        return float(c_inj.get('default_fill_efficiency', 0.60)), 'default'
    ...
    def _compute_suckback_distance(
        ...
        family = parse_material_family(abbreviation)
    ...
    def _derive_process(self, params: ProductionParams):
        ...
        total_len = compute_total_injection_length(total_weight, melt_density, screw_diameter)
        ...
        cushion_len = compute_cushion_length(screw_diameter)
        ...
        family = parse_material_family(mat.get('abbreviation', ''))
        ...
        logger.debug(
            ...
            f"inj_pres={inj_pres:.2f}MPa ({('%.1f' % (inj_pres / max_inj_pres * 100)) if max_inj_pres > 0 else 'N/A'}% max)"
        )
        ...
        logger.debug(
            ...
            f"inj_velo={inj_velo:.2f}mm/s ({('%.1f' % (inj_velo / max_inj_velo * 100)) if max_inj_velo > 0 else 'N/A'}% max)"
        )
        ...
        # 步骤 1：基于界面最大值 + family 填充效率推导等效速度（自包含，不依赖 #4 inj_velo）
        # 物理含义：实际填充速度 = 机器能提供的最大速度 × family 填充效率
        # 例如 PP fill_efficiency=0.85 → equivalent_velo = max × 0.85（接近 max）
        #       PC fill_efficiency=0.40 → equivalent_velo = max × 0.40（必须慢）
        # 重构动机：原设计依赖 #4 inj_velo输出（混合模式），现采用 family 填充效率驱动（系数模式）
        # 优点：不依赖 #4 inj_velo输出 → 鲁棒性提升；与 #3 #4 #6 #7 等系数模式一致 → 认知一致
        fill_efficiency, fill_level = self._get_family_fill_efficiency(family, c_inj)
        equivalent_velo = max_inj_velo * fill_efficiency

        # 步骤 2：计算几何下限（用等效速度，不用 #4 inj_velo）
        # 等效速度=0 时退化到 min_t（避免除零）
        inj_time_geo = inj_len / equivalent_velo if equivalent_velo > 0 else min_t

        # 步骤 3：钳制：保留物理可达性（≥ min_t 防冻结、≤ max_t 防飞边/欠注）
        inj_time = max(min_t, min(inj_time_geo, max_t))

        logger.debug(
            f"注射时间: family={family or 'unknown'} (fill_level={fill_level}, fill_efficiency={fill_efficiency:.3f}), "
            f"max_thickness={max_thickness:.2f}mm, bucket={bucket} (level={bucket_level}), "
            f"window=({min_t:.2f}s, {max_t:.2f}s) (level={window_level}), "
            f"max_inj_velo={max_inj_velo:.2f}mm/s, equivalent_velo={equivalent_velo:.2f}mm/s, "
        ...
        abbreviation = mat.get('abbreviation', '')
        family = parse_material_family(abbreviation) if abbreviation else ''
