...
from .utils import drop_chars
from .utils import weighted_avg_freqs


class Ruleset:
    """Collection of RuleMethod in disjunctive normal form."""
    ...
    def __eq__(self, other):
        ...
        for r in self.rules:
            # Ideally, should implement a hash function--in practice speedup would be insignificant
    ...
    def trim_conds(self, max_total_conds=None):
        """.Reduce the total number of Conds in a Ruleset by removing RuleMethod."""
    ...
    def trimmed_str(iterable, max_items=3):
        # unexpected type Py_UCS4 and base type Python object for indexing 
        # return str(iterable[:max_items])[-1] + "..."
        trimmed = iterable[:max_items]
        return str(trimmed) + "..." if len(iterable) > max_items else str(trimmed)
    ...
    def predict(self, X_df, give_reasons=False, warn=True):
        ...
        If give_reasons is True, returns a tuple that contains the above list of predictions
            ...
            for each positive prediction, gives a list of one-or-more covering RuleMethod, for negative predictions, an empty list.
        """
        ...
        if possible_conds is not None:
            ...
            for feat in pos_df.columns.values:
                for val in set(pos_df[feat].unique()).intersection(
                    ...
                    ):  # Conds already in Rule and Conds that contradict Rule aren't valid successors / NB RuleMethod are short; this is unlikely to be worth the overhead of cheacking
...
def asruleset(obj):
    ...
    if type(obj) == Ruleset:
        ...
        return obj.ruleset_
    elif hasattr(obj, "fit") and "induction" in str(obj.__class__):
        return Ruleset()
    elif obj is None or not obj:
        return Ruleset()
    elif hasattr(obj, "__iter__"):
        try:
            ...
            raise TypeError(
                f"Couldn't interpret {obj} type {type(obj)} as Ruleset. Type should be Ruleset, a induction ruleset classifier, list of RuleMethod, or str"
            )
    else:
        raise TypeError(
            f"asruleset: {obj} type {type(obj)} cannot be converted to Ruleset. Type should be Ruleset, a induction ruleset classifier, list of RuleMethod, or str"
        )
