...
from .check import _check_is_model_fit, _warn
from .preprocess import BinTransformer, _upgrade_bin_transformer_ifdepr
...
class AbstractRulesetClassifier(ABC):
    ...
    def predict(self, X, give_reasons=False, feature_names=None):
        ...
        tuple<list<bool>, <list<list<Rule>>>
            ...
            for each positive prediction, a list of all the covering RuleMethod, for negative predictions, an empty list.
        """
    ...
    def recalibrate_proba(
        ...
        require_min_samples : bool, default=True
            True: halt (with warning) in case min_samples not achieved for all RuleMethod
            False: warn, but still replace RuleMethod that have enough samples
    ...
    def _ruleset_frommodel(self, model):
        """Return the ruleset from model, which may be a Ruleset, induction classifier, or str."""
        if not model:
            return Ruleset()
            ...
            raise AttributeError(
                f"Couldnt recognize type: {type(model)} of model: {model}. Model should be of type Ruleset, str defining a ruleset, or induction classifier."
            )
