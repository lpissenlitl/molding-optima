...
def interpret_model(
    ...
    """Interpret another model using a induction interpreter as global surrogate.
        ...
        interpreter : IREP or RIPPER object, default=RIPPER()
            induction classifier to perform interpretation.
...
def score_fidelity(
    ...
    interpreter : IREP or RIPPER object, default=RIPPER()
        induction classifier to perform interpretation.
    model : trained sklearn, keras, pytorch, or induction, etc. classifier, default=None
...
def model_predict(X, model, model_predict_function=None):
    ...
    if not model_predict_function:
        ...
        elif _inpackage(model, "induction"):
