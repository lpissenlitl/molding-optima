...
class User(BaseModel):
    ...
    def is_accessible(self):
        ...
        if self.is_tenant_admin:
            return self.company.is_accessible()  # 租户管理员用户，权限依赖于公司状态
        if self.is_deleted:
            return False  # 删除用户，无访问权限
        if not self.is_active:
            return False  # 未激活用户，无访问权限
        if self.expires_at and self.expires_at < datetime.now():
            return False  # 过期用户，无访问权限
        return True
    ...
    def set_permissions(self, permission_codes: list):
        ...
        if added_codes: 
            RolePermission.objects.bulk_create([
                RolePermission(role=self, permission_code=code) for code in added_codes
            ])        
        
    def get_permissions(self):
        """返回角色权限 code 列表"""
        # return [ e.permission_code for e in self.permissions.all() ]
...
class RedirectToken(TracedModel):
    ...
    @classmethod
    def create(cls, user, target_url, expire_hours=72):
        """生成一个72小时内有效的跳转token"""
