export function conversion(original_unit: string, converted_unit: string, original_value: number, injector_info: any, convert_type?:string) {
    ...
    if (!original_value) {
        ...
        if (original_unit == "mm/s" && converted_unit == "inch/s") {
            ...
            return (original_value * 25.4).toFixed(VALID_NUMBER)
        }else if (original_unit == "inch/s" && converted_unit == "cm3/s") {
            return (original_value * 25.4 * getScrewArea(injector_info) / 1000).toFixed(VALID_NUMBER)
        }else if (original_unit == "cm3/s" && converted_unit == "inch/s") {
            return (original_value / 25.4 / getScrewArea(injector_info) * 1000).toFixed(VALID_NUMBER)
        ...
        }else if (original_unit == "inch³/s" && converted_unit == "cm3/s" && getScrewArea(injector_info)) {
            ...
            // 在速率转换%时,需要用到最大速率,需要convert_type来区分是保压,注射还是射退, 单位是cm3/s
            ...
            // console.log("转前{original_unit} 转后{converted_unit} 最大速度{max_open_velocity} 当前值{original_value}")
            ...
            // 螺杆转速,需要用到螺杆周长
            // console.log("转前"+original_unit+" 转后"+converted_unit+"转后的数值"+original_value/getScrewCircumference(injector_info)*10*60)
            return (original_value / getScrewCircumference(injector_info) * 10 * 60).toFixed(VALID_NUMBER)
        } else if (original_unit == "cm/s" && converted_unit == "%") {
            // 最大可设定螺杆转速线速度 单位 是cm/s
            // console.log(injector_info.max_screw_linear_velocity)
            // console.log(injector_info.max_screw_rotation_speed)
            if(injector_info.max_screw_linear_velocity) {
                ...
                let max_screw_linear_velocity:any = conversion("rpm", "cm/s", injector_info.max_screw_linear_velocity, injector_info)
...
export function getScrewCircumference(injector_info: any) {
    if (injector_info.screw_diameter) {
        ...
        // console.log("螺杆直径为空?{injector_info.screw_diameter }")
