...
function getProjects() {
  const projects = getLocalStorageObject("list_of_project")
  if (projects) {
    return projects
  } else {
    return []
...
function getSelectedTestingIndexById(project: any, trial_session_id: number) {
  for (let i = 0; i < project.testing_list.length; ++i) {
    if (trial_session_id == project.testing_list[i].id) {
      return i
    }
  }
  return -1
...
export function getSelectedProjectById(project_list: any, project_id: number) {
  const index = getSelectedProjectIndexById(project_list, project_id)
...
class ProjectsInfo extends VuexModule implements IProjectsInfo {
  ...
  private UPDATE_PROJECT(project: any) {
    const project_index = getSelectedProjectIndexById(this.projects, project.id)
    if (project_index >= 0) {
      this.projects[project_index] = project
      saveLocalStorageObject("list_of_project", this.projects)
  ...
  private REMOVE_PROJECT(project: any) {
    ...
    if (project_index >= 0) {
      ...
      if (this.projects.length == 0) {
        removeLocalStorage("list_of_project")
      } else {
        saveLocalStorageObject("list_of_project", this.projects)
      }
      ...
      removeLocalStorage("selected_project")
      this.selectedTesting = null
      removeLocalStorage("selected_testing")
      this.selectedNode = null
      removeLocalStorage("selected_node")
  ...
  public PUSH_TESTING(testing: any) {
    const project_index = getSelectedProjectIndexById(this.projects, testing.project_id)
    if (project_index >= 0) {

      const testing_index = getSelectedTestingIndexById(this.projects[project_index], testing.id)
      if (testing_index == -1) {
        this.projects[project_index].testing_list.push(testing)
        saveLocalStorageObject("list_of_project", this.projects)

        // 设置当前的工程和测试
        this.selectedProject = this.projects[project_index]
        saveLocalStorageObject("selected_project", this.selectedProject)
  ...
  public UPDATE_TESTING(testing: any) {
    ...
    if (project_index >= 0) {
      ...
      if (testing_index >= 0) {
        this.projects[project_index].testing_list[testing_index] = testing
        saveLocalStorageObject("list_of_project", this.projects)
  ...
  public REMOVE_TESTING(testing: any) {
    const project_index = getSelectedProjectIndexById(this.projects, testing.project_id)
    if (project_index >= 0) {
      const index = this.projects[project_index].testing_list.findIndex((d: any) => d.id === testing.id)
      this.projects[project_index].testing_list.splice(index, 1)
      saveLocalStorageObject("list_of_project", this.projects)
  ...
  private SET_SELECTED_NODE(node: any) {
    ...
    if (node == null) {
      ...
      const project_index = getSelectedProjectIndexById(this.projects, node.pro_id)
      if (project_index >= 0) {
        const testing_index = getSelectedTestingIndexById(this.projects[project_index], node.trial_session_id)

        if (testing_index >= 0) {
          this.selectedProject = this.projects[project_index]
          saveLocalStorageObject("selected_project", this.selectedProject)

          this.selectedTesting = this.projects[project_index].testing_list[testing_index]
          saveLocalStorageObject("selected_testing", this.selectedTesting)
...
export const ProjectsModule = getModule(ProjectsInfo)
