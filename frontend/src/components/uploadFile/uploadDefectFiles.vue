...
export default {
  ...
  methods: {
    beforeUpload (file) {
      ...
      // if (!isJpg && !isPng && !isJpeg && !isPdf && !isPpt&& !isWord) { 
      //     this.$message.error('只能上传png或jpg格式或pdf格式或ppt格式或doc格式的文件!');
      //     return false;
      // }
      if (!isLt20M) {
