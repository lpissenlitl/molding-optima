<template>
  <div ref="processOtimize">
      ...
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          ...
          <span style="color:red"> 
            <span v-if="optimize_detail.feedback_detail.optimize_export.rule_library_in_use ">
              当前规则库:{{ optimize_detail.feedback_detail.optimize_export.rule_library_in_use }} 
            </span>
            <span v-if="optimize_detail.feedback_detail.optimize_export.rule_in_use ">
              当前规则：{{ optimize_detail.feedback_detail.optimize_export.rule_in_use }} </span>
            </span>
        </div>
        ...
        >
          ...
          <template v-for="(defect, index) in optimize_detail.feedback_detail.defect_info">
            <el-form-item :label="defect.label" :prop="defect.prop" :key="index">
              <template #label>
                <span @click="handleClick(defect.label)">{{ defect.label }}</span>
              </template>
              <el-select
              ...
              <el-button
                type="primary"
                size="mini"
                @click="checkDetail(defect.label)"
                v-if="defect.level !== '无缺陷'"
                style="width: 4rem"
              >
                详情
              </el-button>
    ...
    >
      <rule-flow
        :graph-data-parent="graphData"
        @close-dialog="closeRuleDialog"
      >
      </rule-flow>
    </el-dialog>
...
export default {
  ...
  methods: {
    handleClick(labelVal) {
      this.showRule = true
      this.graphData = {}
      if (this.precondition_detail.product_type && this.precondition_detail.polymer_abbreviation) {
        getRuleFlowMethod({"rule_type": "子规则库", " polymer_abbreviation": this.precondition_detail.polymer_abbreviation, "product_small_type": this.precondition_detail.product_type}).then(res => {
          if (res.status === 0 && JSON.stringify(res.data) != "{}" && res.data.defect_data) {

            for (let i = 0; i < res.data.defect_data.length; i++) {
              if (res.data.defect_data[i].defect_name == labelVal) {
                this.graphData.nodes = res.data.defect_data[i].graph_data.nodes;
                this.graphData.edges = res.data.defect_data[i].graph_data.edges;
              }
            }
          }
    ...
    setParamDetail(){
      let inject_data = this.optimize_detail.process_detail.inject_para.table_data
      let holding_data = this.optimize_detail.process_detail.holding_para.table_data
      let metering_data = this.optimize_detail.process_detail.metering_para.table_data
      this.param_detail = [
        {
          category: "压力",
          desc: "注射一段压力",
          set_value: inject_data[0].sections[0],
          actual_value: null,
          range: "0-200",
          result: null,
        },
        {
          category: "压力",
          desc: "保压一段压力",
          set_value: holding_data[0].sections[0],
          actual_value: null,
          range: "0-140",
          result: null,
        },
        {
          category: "压力",
          desc: "背压",
          set_value: metering_data[2].sections[0],
          actual_value: null,
          range: "10-50",
          result: null,
        },
        {
          category: "时间",
          desc: "注射时间",
          set_value: this.optimize_detail.process_detail.inject_para.injection_time,
          actual_value: null,
          range: "0-20",
          result: null,
        },
        {
          category: "时间",
          desc: "保压一段时间",
          set_value: holding_data[2].sections[0],
          actual_holding_time: null,
          range: "0-10",
          result: null,
        },
        {
          category: "速度",
          desc: "注射一段速度",
          set_value: inject_data[1].sections[0],
          actual_value: null,
          range: "0-120",
          result: null,
        },
        {
          category: "速度",
          desc: "保压一段速度",
          set_value: holding_data[1].sections[0],
          actual_value: null,
          range: "0-100",
          result: null,
        },
        {
          category: "温度",
          desc: "温度",
          set_value: this.optimize_detail.process_detail.temp_para.table_data[0].sections[0],
          actual_value: null,
          range: "150-350",
          result: null,
        },
